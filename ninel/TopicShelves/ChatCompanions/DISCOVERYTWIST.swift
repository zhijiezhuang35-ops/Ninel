...
struct DISCOVERYTWIST: View {
    ...
    @State private var whisperShade = ThreadWeave.sharedLedger.whisperFocus()
    ...
    var body: some View {
            ...
            VStack(spacing: 0) {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 0) {
                        HStack(spacing: 16) {
                            WhisperBadge(badgeWhisper: whisperShade?.badgeWhisper ?? "hue:FF5538", badgeSize: 76)
                            VStack(alignment: .leading, spacing: 10) {
                                Text(whisperShade?.aliasEcho ?? "入力してください")
                                    .font(.system(size: 18, weight: .bold))
                                    .foregroundStyle(.white)

                                Text(whisperShade?.asideThread ?? "入力してください")
                        ...
                        if let whisperShade {
                            ForEach(ThreadWeave.sharedLedger.petals(for: whisperShade), id: \.pageInk) { threadPetal in
                                ThreadPanel(threadPetal: threadPetal)
                                    .padding(.top, 12)
                            }
                        }
                        ...
                        Button {
                            signalRipple.holdEcho("削除中") {
                                if let whisperShade {
                                    ThreadWeave.sharedLedger.whisperDrop(whisperShade.echoInk)
                                }
        ...
        .onAppear {
            whisperShade = ThreadWeave.sharedLedger.whisperFocus()
        }
    }
}
